...
| Portal (aplicação) | `portal-samba` | 5000/tcp (só localhost) | Flask + gunicorn |
...
| `/opt/portal-samba` | Aplicação do portal (app.py, config.py, venv) |
...
portal:
  dir:  /opt/portal-samba     # onde a aplicação vive
...
```bash
systemctl stop smbd nmbd portal-samba          # libera o /mnt/raid
...
systemctl start smbd nmbd portal-samba
```
...
fail2ban-client status portal-samba        # IPs banidos na jail do portal
#   └─ regra: 5 logins errados em 5 min → banido por 30 min
fail2ban-client set portal-samba unbanip 10.14.29.50   # desbanir na mão
```

A jail lê `/var/log/portal_samba_access.log`: login que falha devolve HTTP 200
...
### 8.1 Unit do systemd anotado (`portal-samba.service`)
...
WorkingDirectory=/opt/portal-samba
ExecStart=/opt/portal-samba/venv/bin/gunicorn --workers 3 --bind 127.0.0.1:5000 \
          --timeout 120 app:app
#         │                        │            │
...
tar -czf /opt/backups/pre_restore_<data>.tar.gz -C / etc/samba etc/passwd etc/shadow etc/group var/lib/samba opt/portal-samba
...
# (se "Portal" foi marcado: systemctl restart portal-samba por último —
...
Portal = `opt/portal-samba` · Compartilhamentos = `mnt/raid/shares` (sobrescreve
...
journalctl -u portal-samba -n 50 --no-pager
#          │                └─ últimas 50 linhas
...
```bash
systemctl status portal-samba nginx --no-pager   # os dois de pé?
ss -tlnp | grep -E '5000|8443'                   # gunicorn no 5000, nginx no 8443?
journalctl -u portal-samba -n 30 --no-pager      # erro do Python aparece aqui
tail -5 /var/log/portal_samba_error.log          # erro do gunicorn/aplicação
