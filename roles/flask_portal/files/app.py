...
RESTORE_INFO_FILE = os.path.join(PORTAL_DIR, '.restore_info.json')
RESTORE_LOG      = '/var/log/cdpni_restore.log'
...
def restore_running():
    """Retorna (is_running, info_dict) para a restauração em andamento.
    Mesma lógica anti-zumbi/anti-reuso-de-pid do backup_running()."""
    try:
        with open(RESTORE_INFO_FILE) as f:
            info = json.load(f)
        pid = info.get('pid')
        if pid:
            try:
                ...
                with open(f'/proc/{pid}/cmdline', 'rb') as fc:
                    cmd = fc.read().decode(errors='ignore')
                if 'restaura' not in cmd and 'tar' not in cmd:
                    return False, info
                return True, info
            except (FileNotFoundError, ProcessLookupError, IndexError):
                return False, info
...
def login():
    ...
    if request.method == 'POST':
        ...
        req_user = user
        p = pam.pam()
        if p.authenticate(user, password, service=PAM_SERVICE):
...
@admin_required
def dashboard():
    svc_list = ['smbd', 'nmbd', 'winbind', PORTAL_NAME, 'nginx']
    services = []
    for s in svc_list:
...
{% endif %}
{% if restore_running %}
<div class="card" style="margin-bottom:1rem;border:2px solid #c0392b">
  <div class="card-header" style="background:#c0392b;color:#fff;display:flex;align-items:center;gap:.5rem">
    <span style="animation:spin 1s linear infinite;display:inline-block">⏳</span>
    <h3 style="color:#fff">Restauração em Andamento</h3>
  </div>
  <div class="card-body">
    <div style="margin-bottom:.5rem">
      <span id="rsElapsed" style="font-family:var(--mono);font-size:1.1rem;font-weight:600">0s</span>
      <span class="text-muted" style="font-size:.82rem"> decorridos &nbsp;·&nbsp; arquivo: <code>{{ rs_info.get('filename','') }}</code></span>
    </div>
    <div class="text-muted" style="font-size:.82rem">Não desligue o servidor. A página recarrega sozinha ao concluir.</div>
  </div>
</div>
{% endif %}
{% if restore_log and not restore_running %}
<div class="card" style="margin-bottom:1rem">
  <div class="card-header"><h3>📜 Última restauração (log)</h3></div>
  <pre class="log-box" style="max-height:170px">{{ restore_log }}</pre>
</div>
{% endif %}
<div class="card" style="margin-bottom:1rem">
    ...
    <form method="post" action="{{ url_for('backup_run') }}" onsubmit="this.querySelector('[type=submit]').disabled=true;this.querySelector('[type=submit]').textContent='Aguarde...'">
      ...
      <div class="form-group" style="margin-top:.5rem">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:.5rem .75rem;margin-top:.25rem">
          ...
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="inc_sambadb" value="1" checked> Banco do Samba — senhas/SID (<code>/var/lib/samba</code>)</label>
          ...
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="inc_portal" value="1" checked> Portal (<code>__PORTAL_DIR__</code>)</label>
        </div>
      </div>
...
<div class="card">
  ...
  <table>
    ...
    <tr>
      ...
      <td class="text-right">
        <button class="btn btn-xs" style="background:#c0392b;color:#fff" onclick="openRestore('{{ b.name }}')" title="Restaurar deste backup">↩ Restaurar</button>
        <a href="{{ url_for('backup_download', filename=b.name) }}" class="btn btn-xs" title="Baixar">⬇</a>
        <button class="btn btn-xs btn-danger" onclick="confirmDelBackup('{{ b.name }}')" title="Excluir">🗑</button>
      </td>
    </tr>
    {% endfor %}
    </tbody>
...
<div class="card" style="margin-top:1rem">
  <div class="card-header"><h3>⬆ Enviar Backup Externo</h3></div>
  <div class="card-body">
    <form method="post" action="{{ url_for('backup_upload') }}" enctype="multipart/form-data"
          style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap"
          onsubmit="this.querySelector('[type=submit]').disabled=true;this.querySelector('[type=submit]').textContent='Enviando...'">
      <input type="file" name="file" accept=".tar.gz" required style="font-size:.82rem">
      <button type="submit" class="btn btn-primary">⬆ Adicionar ao histórico</button>
    </form>
    <small class="text-muted">Para restaurar um backup guardado fora do servidor (ex.: baixado numa máquina Windows). Somente <code>.tar.gz</code> gerado pelo backup do portal — limite 512&nbsp;MB.</small>
  </div>
</div>
<div class="modal-bg" id="mRestore">
  <div class="modal" style="min-width:480px;max-width:640px">
    <div class="modal-title">
      <h3>↩ Restaurar Backup</h3>
      <button type="button" class="modal-close" onclick="closeModal('mRestore')">&times;</button>
    </div>
    <form method="post" action="{{ url_for('restore_run') }}" onsubmit="return confirmRestore(this)">
      <div style="padding:1rem">
        <div style="font-size:.82rem;margin-bottom:.75rem">Arquivo: <code id="rstFileLabel" style="font-size:.78rem"></code></div>
        <input type="hidden" name="filename" id="rstFile">
        <label style="font-size:.82rem">O que restaurar</label>
        <div style="display:flex;flex-direction:column;gap:.4rem;margin-top:.35rem">
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="rst_samba" value="1" checked> Configuração Samba (<code>/etc/samba</code>)</label>
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="rst_sambadb" value="1" checked> Banco do Samba — senhas/SID (<code>/var/lib/samba</code>)</label>
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="rst_users" value="1"> Usuários/Grupos (<code>/etc/passwd, shadow, group</code>) — cuidado: sobrescreve contas criadas depois do backup</label>
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="rst_portal" value="1"> Portal (<code>__PORTAL_DIR__</code>) — o portal reinicia ao final</label>
          <label style="font-weight:400;font-size:.83rem"><input type="checkbox" name="rst_shares" value="1"> Compartilhamentos (<code>{{ samba_root }}</code>) — sobrescreve os arquivos atuais com as versões do backup (arquivos criados depois são mantidos)</label>
        </div>
        <div style="background:var(--bg3);border-radius:6px;padding:.6rem .75rem;margin-top:.75rem;font-size:.78rem" class="text-muted">
          🛟 Antes de aplicar, as configurações atuais são salvas automaticamente num
          <code>pre_restore_*.tar.gz</code> no histórico — dá para desfazer restaurando ele.
          Itens que não existirem dentro do arquivo são apenas avisados no log.
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn" onclick="closeModal('mRestore')">Cancelar</button>
        <button type="submit" class="btn" style="background:#c0392b;color:#fff">↩ Restaurar</button>
      </div>
    </form>
  </div>
</div>
...
function openRestore(n) {
  document.getElementById("rstFile").value = n;
  document.getElementById("rstFileLabel").textContent = n;
  document.getElementById("mRestore").classList.add("open");
}
function confirmRestore(f) {
  if (!f.querySelector("input[type=checkbox]:checked")) {
    alert("Selecione ao menos um item para restaurar.");
    return false;
  }
  return confirm("Restaurar os itens selecionados de \"" +
    document.getElementById("rstFile").value + "\"?\n\n" +
    "As configurações atuais serão sobrescritas (um snapshot de segurança será criado antes).");
}
(function() {
  var running = {{ "true" if restore_running else "false" }};
  if (!running) return;
  function fmtTime(s) {
    if (s < 60) return s + "s";
  ...
  var timer = setInterval(function() {
    fetch("{{ url_for('restore_status') }}")
      .then(function(r) { return r.json(); })
      .then(function(d) {
        if (d.running) {
          var el = document.getElementById("rsElapsed");
      ...
      .catch(function() {});   // portal reiniciando ao final: ignora e tenta de novo
  }, 2000);
})();
...
# O template acima e string crua, nao f-string: o caminho do portal entra
# por placeholder para acompanhar o nome definido no Ansible.
BACKUPS_T = BACKUPS_T.replace('__PORTAL_DIR__', PORTAL_DIR)
...
def backups_page():
    ...
    rs_running, rs_info = restore_running()
    restore_log = ''
    if os.path.exists(RESTORE_LOG):
        _, saida, _ = run(['sudo', 'tail', '-n40', RESTORE_LOG])
        blocos = re.split(r'(?=^=== )', saida, flags=re.M)
        restore_log = blocos[-1].strip() if blocos else saida
    return render_template_string(BACKUPS_T,
        ...
        restore_running=rs_running, rs_info=rs_info, restore_log=restore_log,
...
def backup_run():
    try:
        ...
        if request.form.get('inc_sambadb'): targets.append('/var/lib/samba')
        if request.form.get('inc_users'):  targets += ['/etc/passwd', '/etc/shadow', '/etc/group']
        if request.form.get('inc_portal'): targets.append(PORTAL_DIR)
        if not targets:
...
def backup_delete():
    ...
    if os.path.exists(path) and os.path.abspath(path).startswith(os.path.abspath(BACKUP_DIR)):
        try:
            ...
            # BACKUP_DIR costuma pertencer ao root (backups criados via sudo tar)
            run(['sudo', 'rm', '-f', path])
...
def backup_upload():
    ...
    if not name.endswith('.tar.gz'):
        flash('O arquivo precisa ser um .tar.gz gerado pelo backup do portal.', 'error')
        return redirect(url_for('backups_page'))
    tmp = os.path.join('/tmp', f'bkupload_{name}')
    try:
        f.save(tmp)
        run(['sudo', 'mkdir', '-p', BACKUP_DIR])
        rc, _, err = run(['sudo', 'mv', tmp, os.path.join(BACKUP_DIR, name)])
        if rc != 0:
            raise RuntimeError(err.strip() or 'falha ao mover o arquivo')
        flash(f'"{name}" adicionado ao histórico — use ↩ Restaurar para aplicá-lo.', 'success')
    except Exception as e:
        try:
            os.unlink(tmp)
        except OSError:
            pass
        flash(f'Erro ao receber o backup: {e}', 'error')
    return redirect(url_for('backups_page'))

@app.route('/admin/backups/restore/status')
@admin_required
def restore_status():
    running, info = restore_running()
    elapsed = int(time.time() - info['started']) if info.get('started') else 0
    return jsonify({'running': running, 'elapsed': elapsed,
                    'filename': info.get('filename', '')})
...
def restore_run():
    import shlex
    bk_run, _ = backup_running()
    rs_run, _ = restore_running()
    if bk_run or rs_run:
        flash('Já existe um backup ou restauração em andamento — aguarde concluir.', 'error')
        return redirect(url_for('backups_page'))

    name = secure_filename(request.form.get('filename', ''))
    bk = os.path.join(BACKUP_DIR, name)
    if not name or not os.path.exists(bk) or \
       not os.path.abspath(bk).startswith(os.path.abspath(BACKUP_DIR)):
        flash('Arquivo de backup não encontrado.', 'error')
        return redirect(url_for('backups_page'))

    alvos = []   # (rótulo, [caminhos relativos à raiz])
    if request.form.get('rst_samba'):   alvos.append(('Configuração Samba', ['etc/samba']))
    if request.form.get('rst_sambadb'): alvos.append(('Banco do Samba', ['var/lib/samba']))
    if request.form.get('rst_users'):   alvos.append(('Usuários/Grupos', ['etc/passwd', 'etc/shadow', 'etc/group']))
    if request.form.get('rst_portal'):  alvos.append(('Portal', [PORTAL_DIR.lstrip('/')]))
    if request.form.get('rst_shares'):  alvos.append(('Compartilhamentos', [SAMBA_ROOT.strip('/')]))
    if not alvos:
        flash('Selecione ao menos um item para restaurar.', 'error')
        return redirect(url_for('backups_page'))

    mexe_samba  = any(r in ('Configuração Samba', 'Banco do Samba') for r, _ in alvos)
    mexe_portal = any(r == 'Portal' for r, _ in alvos)

    ts   = datetime.now().strftime('%Y%m%d_%H%M%S')
    snap = os.path.join(BACKUP_DIR, f'pre_restore_{ts}.tar.gz')
    # Snapshot de segurança: SÓ configurações (nunca os shares — grandes demais);
    # é o "desfazer" da restauração.
    snap_paths = [p for p in ('etc/samba', 'etc/passwd', 'etc/shadow', 'etc/group',
                              'var/lib/samba', PORTAL_DIR.lstrip('/'))
                  if os.path.exists('/' + p)]

    q = shlex.quote
    linhas = [
        f'exec >>{q(RESTORE_LOG)} 2>&1',
        f'echo "=== $(date "+%d/%m/%Y %H:%M:%S") restauração de {name} ==="',
        f'BK={q(bk)}',
        # aceita membros gravados como "etc/samba" (padrão atual) ou "./etc/samba"
        'rst() { tar -xzf "$BK" -C / "$1" 2>/dev/null || tar -xzf "$BK" -C / "./$1" 2>/dev/null; }',
    ]
    if snap_paths:
        linhas.append(
            f'tar -czf {q(snap)} -C / ' + ' '.join(q(p) for p in snap_paths) +
            f' && echo "OK: configurações atuais salvas em {os.path.basename(snap)} (use-o para desfazer)"'
            f' || echo "AVISO: falha ao criar o snapshot de segurança"')
    if mexe_samba:
        linhas.append('systemctl stop smbd nmbd 2>/dev/null; echo "Samba parado para a restauração"')
    for rotulo, caminhos in alvos:
        for p in caminhos:
            linhas.append(f'rst {q(p)} && echo "OK: /{p} restaurado ({rotulo})"'
                          f' || echo "AVISO: /{p} não está dentro do arquivo de backup"')
    if any(r == 'Configuração Samba' for r, _ in alvos):
        linhas.append('testparm -s >/dev/null 2>&1 && echo "OK: smb.conf validado (testparm)"'
                      ' || echo "AVISO: o testparm apontou problema no smb.conf restaurado"')
    if mexe_samba:
        linhas.append('systemctl start smbd nmbd && echo "OK: Samba reiniciado"')
    linhas.append('echo "OK: restauração concluída"')
    if mexe_portal:
        # por último: derruba este processo — o flash/redirect já foi entregue
        linhas.append(f'echo "Reiniciando o portal..."; sleep 1; systemctl restart {PORTAL_NAME}')

    proc = subprocess.Popen(['sudo', 'bash', '-c', '\n'.join(linhas)],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                            start_new_session=True)
    pgid = os.getpgid(proc.pid)
    with open(RESTORE_INFO_FILE, 'w') as fp:
        json.dump({'pid': proc.pid, 'pgid': pgid, 'filename': name,
                   'started': time.time(), 'itens': [r for r, _ in alvos]}, fp)
    flash(f'Restauração de "{name}" iniciada — acompanhe o log nesta página.', 'success')
    return redirect(url_for('backups_page'))
...
def change_pass():
    ...
    p = pam.pam()
    if not p.authenticate(user, current_pass, service=PAM_SERVICE):
